...
class NFBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, expansion: float = 0.5,
        ...
        self.use_stochdepth = stochdepth_rate is not None and 0. < stochdepth_rate < 1.
    ...
    def forward(self, x):
        ...
        if shortcut.shape[2] != out.shape[2]:
            out = nn.Upsample((shortcut.shape[2], shortcut.shape[3]), mode='bilinear')(out)
        return out * self.alpha * self.skip_gain + shortcut
        # return out + shortcut


class NFBlockCustom(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, expansion: float = 0.5,
                 se_ratio: float = 0.5, stride: int = 1, beta: float = 1.0, alpha: float = 0.2,
                 group_size: int = 1, stochdepth_rate: float = None, activation: str = 'gelu'):

        super(NFBlockCustom, self).__init__()
        ...
        # self.conv0 = WSConv2D(in_channels=self.in_channels, out_channels=self.width, kernel_size=1)
        self.conv1 = WSConv2D(in_channels=self.in_channels, out_channels=self.width, kernel_size=3, stride=stride,
                              groups=self.groups)
        # self.conv1b = WSConv2D(in_channels=self.width, out_channels=self.width, kernel_size=3, stride=1, padding=1,
        #                        groups=self.groups)
        self.conv2 = WSConv2D(in_channels=self.width, out_channels=self.out_channels, kernel_size=1)
        ...
        if self.use_projection:
            ...
            self.conv_shortcut = WSConv2D(self.in_channels, self.out_channels, kernel_size=1)
        # else:
        #     self.conv_shortcut = WSConv2D(self.in_channels, self.out_channels, kernel_size=3)
    ...
    def forward(self, x):
        # print(f"Stride of this block {self.stride}")
        # print(f" use projection {self.use_projection}")
        out = self.activation(x) * self.beta
        # print(f"before shortcut {out.shape}")
        if self.stride > 1:
            shortcut = self.shortcut_avg_pool(out)
            shortcut = self.conv_shortcut(shortcut)
            # print('use shortcut')
        ...
        out = self.conv2(out)
        out = (self.squeeze_excite(out) * 2) * out
        # print(f"without shortcut {out.shape}")
